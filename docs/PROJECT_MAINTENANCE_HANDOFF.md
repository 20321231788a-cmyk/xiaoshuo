...
> 更新：2026-08-04
>
> 当前版本：`1.3.0`（GitHub 正式 Release）
>
> 定位：免费、小范围使用的 AI 小说写作正式版；本版本跳过 RC，仅面向小规模用户使用，不代表商业化质量承诺

## 1. 当前结论

1.3.0 在 1.2.0 基线上修复了 Model Gateway 丢弃思考增量的问题，并完成独立任务线程、领域数据真实写入回执、分卷管理、长任务进度和轻量任务模型的收口。普通 Desktop 启动默认启用集成配置，`--safe-agent` 可一键回退。
...
- `1.2.0` 与 `1.3.0` 为明确批准的例外：跳过 RC，使用小规模正式发布工作流创建非预发布 GitHub Release；Release 说明必须明确“小规模使用、可能未签名、非商业稳定版”，不能伪装成已完成商业发布门槛的版本。后续版本默认仍按 `docs/RELEASE_GATES.md` 的 RC/正式晋升流程执行，除非用户再次明确授权同类例外。
