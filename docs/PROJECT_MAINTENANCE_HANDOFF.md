...
> 更新：2026-08-04
>
> 当前版本：`1.3.1`（GitHub 正式 Release）
>
> 定位：免费、小范围使用的 AI 小说写作正式版；本版本跳过 RC，仅面向小规模用户使用，不代表商业化质量承诺

## 1. 当前结论

1.3.1 在 1.3.0 基线上补齐了新建项目的身份登记、AI 参考资料确认面板与会话管理入口，避免项目路径冲突被误显示为已可写入的确认卡。普通 Desktop 启动默认启用集成配置，`--safe-agent` 可一键回退。
...
- `1.2.0`、`1.3.0` 与 `1.3.1` 为明确批准的例外：跳过 RC，使用小规模正式发布工作流创建非预发布 GitHub Release；Release 说明必须明确“小规模使用、可能未签名、非商业稳定版”，不能伪装成已完成商业发布门槛的版本。后续版本默认仍按 `docs/RELEASE_GATES.md` 的 RC/正式晋升流程执行，除非用户再次明确授权同类例外。
...
2026-08-04 1.3.0 流式通道、任务隔离与工作台可靠性收口：

- Model Gateway 新增带通道的流式转发，完整保留 `reasoning` 与 `answer` 增量，并将思考强度透传给主路线模型；桌面 IPC、NDJSON 事件和会话记录均已端到端验收。
- 拆书、融梗、蒸馏和正文续写均使用专属任务线程，不再复用普通 AI 助手会话；停止、进度、失败信息与恢复入口按任务隔离。
- 领域写入统一提交到故事规划与资料库主数据，确认完成后刷新故事大纲、风格、题材和设定资料页面；普通文件写入不再伪装为结构化保存成功。
- 故事大纲补齐分卷新建、编辑、排序、章节归属和安全删除；拆书工作台、章节批量生成补齐即时状态、进度条和恢复控制。
...
- 修复 Windows 短暂文件占用导致生成缓存元数据无法原子替换的问题；`EPERM`、`EBUSY`、`EACCES` 会有限重试，缓存恢复测试连续执行 20 次通过。
- 本地发布前验证：131 个 Vitest 文件、968 项测试；3/3 Node 测试；Workbench/Desktop production build；15/15 Browser E2E；Desktop smoke；`git diff --check`。
...
2026-08-04 1.3.0 发布后会话与项目写入维护：

- AI 助手会话标题栏取消折叠菜单，直接提供标题编辑、会话摘要与二次确认删除；新建会话不再出现无删除入口的状态。
- 首次发送遇到多份候选参考资料时，输入区上方会显示可操作的确认面板，可勾选候选、直接发送或取消；空白项目模板不会再触发无意义的参考资料确认。
- 新项目创建完成后立即登记 manifest UUID 与物理目录身份。仅在用户明确的新建或重新打开动作中，允许替换已被重新创建目录占用的旧路径记录；后台写入仍保持拒绝未确认身份。
- 项目资料草稿加载失败会显示“待确认写入不可用”和真实错误，不再把路径 UUID 冲突误呈现为可确认的写入卡片。
- 此为 `v1.3.0` 正式标签后的维护提交，不改写现有 GitHub Release、安装包或标签。后续需要让安装包包含该提交时，应发布新版本标签，不得覆盖 `v1.3.0`。
- 定向验证通过：49 项 Vitest 测试、Agent Runtime/Desktop Shell/Workbench 类型检查、Workbench production build，以及会话管理与参考资料确认两条 Browser E2E；`git diff --check` 通过。

2026-08-04 1.3.1 小范围正式发布：

- 将 `1.3.0` 正式标签后的会话与项目写入修复滚动发布为 `1.3.1`，不覆盖既有的 `v1.3.0` 标签、安装包或 GitHub Release。
- 已授权 `v1.3.1` 使用小范围正式发布例外：从带注释标签构建，执行常规发布验收并发布非预发布 GitHub Release；严格 RC、密封评估与商业发布证据仍不在本次声明范围内。
- 发布工作流仅允许 `v1.2.0`、`v1.3.0`、`v1.3.1` 走该例外路径；其他版本继续受严格 RC/溯源发布门禁约束。
- 本地完整发布前验收通过：全 workspace 类型检查、131 个 Vitest 文件/972 项测试、3 个 Node 脚本测试、8 组评估、Workbench/Desktop production build、16/16 Browser E2E、Desktop smoke 与 `git diff --check`。
