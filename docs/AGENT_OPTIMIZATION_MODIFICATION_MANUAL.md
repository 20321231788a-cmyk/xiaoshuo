...
>
> 方案修订：2026-07-13 效率优先的双批次实施与一次总验收
>
...
本文不是概念清单。每个阶段都包含工程目标、边界、测试覆盖和回滚要求；测试用例随代码补齐，但当前剩余工作统一在 Batch A、Batch B 全部完成后执行一次总验收，不在小改动、单个测试文件或单批次出口重复运行验收。每个阶段包含：
...
这组结果只代表 0.4.0 代码基线，不代表当前工作树。15.0 保留 P0 历史台账，当前实现状态和剩余执行顺序以 20.1、20.2 为准。
...
截至 2026-07-10，P0 草稿已经证明 schema/store/coordinator 路径可行，但当时仍存在恢复、暂停、确认、commit journal 接线、认证、Workbench 构建和发布基础设施缺口；这是历史审查结论，不代表当前工作树状态。现状与剩余工作统一以第 20 节为准，且在 M7 完成前仍不具备发布条件。

## 2. ArcWriter 0.4.0 历史基线问题与优先级

本节只描述已提交的 ArcWriter 0.4.0 基线，用于解释 P0～P7 的设计来源；本节中的“当前”“尚未”“仍需”均指 2026-07-10 审阅时的 0.4.0 基线，不用于判断 2026-07-13 工作树。当前实现状态和剩余工作以第 20 节为准。

本节使用 `S0/S1/S2` 表示当时的问题严重级别，避免与后文实施阶段 `P0-P7` 混淆。`S0` 是可靠闭环前置，`S1` 是智能质量核心，`S2` 是产品化与规模化门槛。
...
本节保留 2026-07-10 P0 实施过程和退出证据，仅用于审计，不再表示“下一轮”执行顺序。当前剩余工作不得按本节 A-H 重新逐项验收，统一执行 20.2 的 Batch A、Batch B 和一次最终验收矩阵。

### 15.0 2026-07-10 P0 实施台账

本历史台账只使用 `未开始 / 实现中 / 已验收`；其中“已验收”表示当时对应 P0 Task 的证据成立，不代表当前工作树已通过最终矩阵，也不代表 RC 或生产就绪。
...
2026-07-10 的 style/genre/lore GeneratedCache 分段保存板块已把两个 HTTP 保存入口收口到同一个 runtime committer：纯 planner 保留旧标题/fenced 解析、style/genre 首目标 fallback、Lore 空占位过滤和 replace 覆盖语义；runtime 只信任缓存元数据决定特殊技能身份，忽略 transport target/save-plan 对固定 section target 的改写，并以稳定 section action key 写 journal。相同目标的多个 save-plan segment 会先折叠成一次最终原子写入，避免首段成功、后段崩溃后重试重复内容。随后 PromptSkillRunner 的 durable 调用改为只生成 pending cache 和受控 deferred descriptor，由 runtime 使用当前 run 的 CommitJournal 提交；直接 `/api/skills/:id/run` 也会创建/重放同一 durable run，故不再走兼容直写。非 durable 的 runner facade 仅保留测试和旧内部调用兼容，F 与 P0 仍为“实现中”。

2026-07-10 的 chat generated-save 板块进一步把 `chat/read_context` 自动保存从 `GeneratedCacheService.commitSavePlan()` 直写改为复用当前 outer run 的 `commitGeneratedCache()`。同步与流式 chat 在生成最终回复后使用 `run_id + step_id + chat_auto_save` 派生稳定 cache；journal 完成后先让 outer run 写入 observation，再统一 mark cache committed 和清理正文。若崩溃发生在 journal finalized 与 outer complete 之间，恢复同一 run 时优先读取 pending cache，不重新调用模型、不重复写目标文件；若 outer run 已 completed 但 cache metadata 收口失败，重复同 `request_id` 或 completed-run 对账会从 observation 重放并补齐 committed/cleanup。该板块只收口自动保存；显式会话 `write_target` 写回、普通文件操作计划和 card draw 仍是 F 的剩余旁路。

2026-07-10 的 conversation write-target 板块继续把显式会话写回从 durable run 之后的 `DocumentService.saveDocument()` 直写移入当前 outer run：`ConversationMessageRequest.write_target` 会进入恢复快照中的 `conversation_write_target/conversation_write_mode/conversation_confirm_write`，runtime 使用 `run_id + step_id + conversation_write_back` 派生确定性 cache，并复用 `commitGeneratedCache()` 写 CommitJournal。同步路径在 complete 前完成 journal 和 system “已写回”消息；流式路径会暂存内部 final，等 journal 完成后再向前端 yield final。若崩溃发生在 journal finalized 与 outer complete 之间，resume 同一 run 会从 pending cache 恢复回复，不重跑模型、不重复写目标文件，也不会重复追加 system 写回消息。P0-F 仍为“实现中”，因为普通 `file_operation` plan 和 card draw 写入仍未全部 journal 化。
2026-07-11 的普通文件操作计划与卡片抽选归档接入 CommitJournal/确认协议板块完成了 P0-F 阶段的最后收尾，至此 Task F 全部验收通过：
- 普通 `file_operation` 计划（包含 planner 规划的操作、直接保存、批量替换）如果在首轮执行时具有 `requiresConfirmation: true`，会在执行副作用前被拦截并挂起，只在批准确认后的第二轮恢复运行时经由 `CommitJournalService` 逐个提交；
- 批量替换支持了在被阻断时生成 `buildBatchReplacePlan` 呈现给前端；同步直调或单元测试中的调用通过 `requiresConfirmation: false` 不受阻断；
- 抽卡挑选（`selectCardDraw`）接入了 CommitJournal 服务，基于 `req_card_draw_select_${drawId}_${candidateId}` 构建 synthetic durable run 事务性写入与物理归档。
- 根级验证：`npm run typecheck`、`npm test`（86 files / 673 tests）和 `npx playwright test tests/e2e/project-entry.spec.ts` 全部绿过。
...
本段只约束最终矩阵之后的未来 RC/放量，不在当前 Batch A、Batch B 之间触发测试。能力成为默认前仍须通过版本化 fixture、shadow 对照和至少 48 小时 internal RC；建立合规 beta channel 后，才使用显式 opt-in 的真实运行证据决定是否成为默认。没有 beta channel、cohort 和隐私合规采集时，不虚构 100/500 用户 run 门槛，以 deterministic/replay、RC soak 和人工配对任务作为证据；样本不足时不因日历到期自动晋级。出现以下任一情况立即停止放量：
...
当前剩余实施采用“两个修改批次、一次最终验收”。Batch A 与 Batch B 可以覆盖同一依赖链上的多个模块、schema、迁移、CI、文档和测试文件；两个批次 scope 冻结后，先完成全部计划内修改，再运行最终矩阵。不得在每次编辑、每个文件、每个新增用例或 Batch A/B 边界重复运行根级 test、build、E2E、Desktop smoke 或 eval。

### 17.1 检查与最终验收

| 类型 | 触发条件 | 执行范围 |
| --- | --- | --- |
| 中途最小检查 | 出现编译阻断、数据损坏/迁移风险或安全 fail-open 疑点 | 只运行受影响 workspace 的编译、单个失败文件/suite 或最小负向用例；不得扩展成回归验收 |
| 最终总验收 | Batch A 与 Batch B 的代码、测试、CI 和文档全部完成 | 按固定顺序运行 17.3 的六条命令一次 |

中途最小检查只用于解除阻断或确认 fail closed，不计为最终验收证据。历史绿码可保留为实现证据，但不得替代当前工作树的最终矩阵。最终矩阵出现失败后，只定向重跑失败命令、失败文件或失败 suite 进行定位；所有已知失败修复完成后，再完整运行最终矩阵一次。安全、身份、预算、确认、数据完整性和 migration 疑似 fail-open 时，必须先用最小负向用例确认 fail closed，才能继续批量修改。

### 17.2 批次交付规则

Batch A 或 Batch B 完成时只做静态交付检查，不运行批次验收：

1. 更新本文对应阶段的实时状态；
2. 准备 `docs/PROJECT_MAINTENANCE_HANDOFF.md` 的合并记录材料，但只在两批完成并结束最终验收后追加一次；
3. 核对计划 scope、迁移、回滚、测试用例和未完成项；
4. 保持可回滚边界，不要求在批次出口提交或跑测试；
5. 将实际变更和待总验收项写入实施记录。

提交不得混入无关 UI、格式化或用户已有文件。

详细 Batch A、Batch B、失败处理和记录模板以 `docs/AGENT_OPTIMIZATION_NEXT_IMPLEMENTATION_MANUAL.md` 为准；若两本手册发生冲突，以本节“一次最终矩阵”规则为当前执行口径。

### 17.3 唯一最终验收矩阵

Batch A 与 Batch B 全部完成后，在同一工作树按以下固定顺序运行一次：
...
通过该矩阵只表示当前优化修改批次达到代码验收基线，不等于完成 L3 RC，不得据此声明已可投产、已生产就绪或 P0-P7 全部完成。clean install、签名、安装/升级/回滚和 soak 仍属于未来独立 RC 门禁。

长期阶段提交命名参考（仅在对应阶段退出条件和所需验收真实满足后使用）：
...
以下能力在 0.5.0～0.9.0 内保持硬禁用，且不可通过任何 Feature Flag 解锁：

- 多 Agent 并行协作（不进入 0.5.0～0.9.0）；
- Agent 自行安装任意工具或库；
...
这些约束是 Negative Capability Gate，不是可由任何 Feature Flag、环境变量、Prompt、网页、附件、项目文件或导入 Skill 开启的候选功能。实现必须 fail closed，并对直接调用、权限伪造、确认重放、路径穿越、symlink/junction、项目切换竞态和 Prompt Injection 建立负向测试。
...
## 20. 实时状态与本轮代码验收（2026-07-13）

2026-07-11 的审阅、旧失败和旧绿码保留在 Git/交接记录中作为历史证据，不再作为当前状态结论。Batch A、Batch B 已完成，并在同一工作树完成 17.3 的完整矩阵复跑；这只证明本轮修改达到代码验收基线，不是“稳定全绿”、RC 候选、可交付生产或 P0-P7 全部完成。

### 20.1 当前实现状态

- M0/M1：文档校准与 Desktop smoke 认证链路已纳入本轮矩阵，`npm run smoke:desktop` 通过；历史绿码不再单独作为结论。
- M2 / P0-P2 集成中（本轮代码验收通过）：`agent_execution_v2_mode` 已覆盖 durable admission/retry/recovery/API，缺少旧路径比较器时 `shadow` 拒绝启用；八个子 Flag 均已有生产消费者和关闭路径。逐 Flag 的 RC 放量、installed-build 和回滚演练仍不在本轮范围。
- M3 / P6 集成中（本轮代码验收通过）：会话计划卡和 Trace 页面均使用真实 durable run metadata/API Client；Browser E2E 覆盖 reload、pause/resume/cancel、409 conflict、retry、确认批准/拒绝。控制后显式回放事件，订阅以事件流加状态校准处理协作式暂停。Browser harness 的 test-only token 不能替代 M7 安装态证据。
- M4 集成中（本轮代码验收通过）：第 19 节 deny-by-default policy、preload/main-process 双层 terminal 授权、canonical realpath/project UUID/跨重启文件 identity guard、Action receipt 和持久预算均已接入；跨项目写入继续直接拒绝。最终 `eval:excluded-capabilities` 生成 110 个 case 的通过 manifest。
- M5 / P3 集成中（本轮代码验收通过）：项目本地 governed memory、confirmation receipt、revision/override/outbox、来源失效、结构化 conversation summary、Workbench 治理入口、图谱/向量投影状态与重建均已接入；100-turn restart 与项目隔离 replay 已实现。它仍需 M7 的完整 RC 证据，不能因本轮矩阵而标记为完成。
- P4 集成中（Batch A 代码验收通过）：`ContextScheduler` 现覆盖稳定 chat 与 prompt-skill 上下文，按 Flag 保留 legacy assembler 回退；模型预算和 memory selector 仍须在 M7 扩展版本化质量数据。
- P5 集成中（Batch A 代码验收通过）：共享 `QualityReport`、hard/subjective 分流、generated cache、文件操作和项目文档的 pre-save gate，以及经用户确认且绑定 eval manifest 的 feedback 候选均已接入；M7 仍需更大质量集和人工校准。
- P7 机制完成、本阶段整体未完成：所有 `eval:*` 命令现在写入 manifest、fixture/case hash、脱敏失败 trace、性能/安全计数，三个 Windows workflow 在成功和失败路径上传 artifact。sealed holdout、数据集规模、clean install、签名、安装/升级/回滚、soak 和人工校准仍为 M7 门槛。

### 20.2 2026-07-13 Batch A + Batch B 集中验收记录

1. **Batch A 已完成。** 收口了 durable run 的 UI 状态同步、P4 在 chat/skill 上下文的 token 调度与 legacy 回退，以及 P5 的统一 pre-save gate、报告和确认反馈路径。
2. **Batch B 已完成。** `scripts/run-eval.mjs` 为每次 eval 生成可复现 manifest 和诊断 artifact；fixture 清单与三个 Windows workflow 的 `always()` artifact 上传已接线。
3. **首次矩阵失败与定向修复。** 前四项通过后，`npm run eval:excluded-capabilities` 在 Windows 因以 `shell: false` 启动 `npx.cmd` 报 `spawn EINVAL`，测试尚未执行。运行器改为用当前 Node 直接调用锁定的 Vitest 入口，并捕获同步 spawn 失败；定向 eval 随后通过。
4. **完整矩阵最终复跑。** `npm run typecheck` 通过；`npm test` 为 103 files / 836 tests；`npm run test:e2e` 为 6/6；`npm run smoke:desktop` 通过（Electron 42.3.0、node-pty、node:sqlite）；`npm run eval:excluded-capabilities` 通过并写出 110 个 case、pass rate 1 的 manifest；`git diff --check` 通过，只有 CRLF 转换预警。

后续只进入 M7 的独立 RC 工作。当前已补齐 previous-release download、升级/回滚 smoke、RC 数据集/20% sealed holdout 声明验证和发布 evidence 的 workflow 门禁，并在本地完成 0.3.2 → 0.4.0 → 0.3.2 演练；本地候选为 dirty workspace 且 `Authenticode=NotSigned`，证据验证器已拒绝它。RC/release 在缺少 `evals/rc-dataset-manifest.json` 或未达到 13 类最低规模时会 fail closed。完整真实数据集、clean install/build、已签名 installed-build、soak 和人工质量校准仍未完成。即使本轮矩阵全绿，当前结论最多为“本轮修改通过代码验收”，不得声称 RC、生产就绪或 0.5.0～0.9.0 已可发布。
