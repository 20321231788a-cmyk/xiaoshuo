...
| F 崩溃/并发/确认 | 实现中 | journal/lease/confirmation；durable direct-save/batch-replace 与拆书所有持久输出的 journal/hash 覆盖；Confirmation UI/API 已通过 | 剩余 Skill/workflow 写入覆盖与 Confirmation 浏览器 E2E |
