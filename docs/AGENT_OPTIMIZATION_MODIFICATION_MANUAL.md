...
- 本地 runtime 只绑定 loopback；每次 runtime 启动生成至少 256-bit 随机令牌，除最小 health check 外所有 API 均鉴权；
- Origin 校验只是纵深防御，不能替代令牌；令牌只通过 Electron 受控启动链路注入，不进入 URL、日志、Trace 或普通 `localStorage`；
- Electron 保持 `contextIsolation=true`、`nodeIntegration=false`，导航、新窗口和权限请求默认拒绝；
...
type AgentGoal = {
  ...
  request_snapshot: {
    ...
    feature_flag_snapshot: Record<string, unknown>;
  };
};
```
...
`request_snapshot` 只保存恢复所需的文本和受控引用，不复制附件全文、API key 或不相关项目内容。run 恢复时使用原始 snapshot 和同一个 `run_id`；只有用户明确修改目标才增加 `goal_revision` 和 `plan_version`。
...
`UNIQUE(run_id, step_id, attempt)` 保证 attempt 单调递增。Observation、模型 usage、artifact 和副作用结果均引用 `attempt_id`；`AgentExecutionStep.attempts` 只是可重建汇总值。
...
数据库固定放在 `{projectRoot}/00_设定集/.agent/agent_runs.sqlite3`，不得与 `vector_index.sqlite3` 或桌面 `xiaoshuo-local-state.sqlite3` 共库。`agent-runtime` 定义最小 `ExecutionDatabase` adapter；desktop-shell 复用现有驱动探测，在 `better-sqlite3` 与 `node:sqlite` 中选择可用实现。业务 store 不直接依赖 Electron，也不把某一个 SQLite 驱动泄漏到 shared 契约。
...
- migration 获取项目级独占锁；未知更高 schema 只读打开并拒绝写入，不尝试降级覆盖；
...
- 恢复完成后执行 `PRAGMA quick_check`、表级计数/hash 校验和 commit journal 对账。
...
- 恢复 run 时复用原 `run_id` 和 request snapshot，将 `failed/paused` 通过 CAS 转为 `running`，再创建新的 step attempt；
- 已完成步骤默认不可重复执行；
- 需要重新执行时生成新 attempt，并保留旧 observation；
- pause 是协作式检查点：设置 `pause_requested_at`，当前不可中断的文件提交完成后再进入 `paused`；
...
- renderer reload 不改变 run；正常应用退出和自动更新重启先停止接收新 run，在期限内写入检查点，超时任务转为 `paused`；
...
- 多文件 action 使用确定顺序和 manifest，任何部分提交必须进入 `recovery_required`/`paused`，不得显示 completed；
...
- Data Disclosure 和 capability/pricing snapshot 固化到 run，运行中修改模型设置不改变已开始的 attempt；
...
- 在线随机模型每个 case 至少运行 3 次，报告均值、最差轮次和 95% CI；
...
发布门禁：P0 schema/migration、状态机、幂等、commit journal、并发、认证、Electron 生命周期、恢复 E2E 和 installed-build smoke 全部通过；关键故障注入累计至少 1000 次，未确认写入、重复副作用和跨项目访问保持为 0。
...
- 结构化会话记忆、user override 和 memory revision；
- 增量向量和图谱更新；
- 一致性接口统一；
- token 预算；
- 语义块选择和去重。
...
下一轮只启动 P0，不同时修改记忆和质量系统。实施按以下垂直切片顺序进行；A-C 完成后立即接 D，不能长期停在“只有 schema/store、真实请求仍不持久”的中间态。
...
- IntentResolution、AgentGoal、AgentRunState、AgentExecutionStep、AgentStepAttempt、AgentObservation、AgentConfirmation、AgentRunEvent；
...
- run/step/attempt/observation/artifact/confirmation/event CRUD；
...
这一切片不追求完整 P2 规划，只把现有执行路径包进可靠生命周期。完成标准是首次出现真实“执行一半 -> 杀进程 -> 同 ID 恢复 -> 完成”的集成测试。
...
| --- | --- | --- | --- | --- | --- |
| off/shadow | off | off | off | off | 旧兼容路径或只记录不双执行 |
...
`context_selector_v2` 可在 `memory_v2` 开启后独立灰度，但关闭时必须回退到旧 ContextAssembler，不能跳过上下文预算。

每个阶段先经过 internal 至少 100 个 run/48h，再进入 beta 至少 500 个 run/7 天，最后才成为默认；样本不足时不因日历到期自动晋级。出现以下任一情况立即停止放量：
...
从 P0 Task A-C 开始，并在契约稳定后立即进入 Task D 最小垂直链路：

1. 先让 shared schema 覆盖 run/goal/step/attempt/confirmation/event/version/error；
2. 通过 adapter 建立项目内 Execution Store 和 migration fixture；
3. 完成状态机、heartbeat、owner lease、operation id 和幂等；
4. 把现有 `runAgent()`/`streamAgentRun()` 接入同一 run coordinator，证明“执行一半 -> 强杀 -> 同 ID 恢复 -> 完成”；
5. 只有该链路通过后，再扩展 commit journal、Workbench 控制、长任务迁移和 P0 发布门禁。
