...
describe("config-service", () => {
  it("matches Python public config defaults", async () => {
    ...
    expect(config).toMatchObject({
      ...
      project_file_permission_mode: "default",
  ...
  it("persists the global project file permission mode and defaults legacy configs safely", async () => {
    expect(normalizePublicConfig({}).project_file_permission_mode).toBe("default");
    const configPath = path.join(tempDir, "studio_config.json");
    const saved = await savePublicConfig({ project_file_permission_mode: "direct_save_delete" }, { configPath });
    expect(saved.project_file_permission_mode).toBe("direct_save_delete");
    expect(JSON.parse(await fs.readFile(configPath, "utf8"))).toMatchObject({ project_file_permission_mode: "direct_save_delete" });
  });
