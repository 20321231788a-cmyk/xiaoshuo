...
const aliases: Record<(typeof publicConfigKeys)[number], string[]> = {
  ...
  project_file_permission_mode: ["project_file_permission_mode", "projectFilePermissionMode"],
...
export function normalizePublicConfig(data: RawConfig): AppConfig {
  ...
  return {
    ...
    project_file_permission_mode: projectFilePermissionModeValue(data.project_file_permission_mode),
    humanizer_enabled: Boolean(data.humanizer_enabled ?? false)
  };
}

function projectFilePermissionModeValue(value: unknown): "default" | "direct_save_delete" {
  return value === "direct_save_delete" ? "direct_save_delete" : "default";
}
