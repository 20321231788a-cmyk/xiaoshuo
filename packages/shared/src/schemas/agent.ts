...
// Some otherwise valid model plans use JSON null for fields that do not apply
// to an action (for example text on archive_file). Treat only these optional
// operation payload fields as empty strings; action and path remain strict.
const nullableOperationTextSchema = z.string().nullable().transform((value) => value ?? "").default("");

export const fileOperationSchema = z
  .object({
    action: z.enum(["create_file", "append_text", "replace_text", "move_file", "archive_file"]),
    path: z.string(),
    text: nullableOperationTextSchema,
    old_text: nullableOperationTextSchema,
    new_text: nullableOperationTextSchema,
    target_path: nullableOperationTextSchema,
    reason: nullableOperationTextSchema,
...
export const generatedSavePlanSchema = z
  .object({
    ...
    // Optional on the wire so older persisted plans and test fixtures remain
    // readable; planners normalize missing values to preview_required.
    write_authorization: z.enum(["preview_required", "explicit_commit"]).optional(),
