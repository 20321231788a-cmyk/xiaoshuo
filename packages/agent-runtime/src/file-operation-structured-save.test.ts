...
describe("AgentFileOperationRunner structured direct saves", () => {
  it("writes a confirmed-outline target into the story-planning master instead of a detached text file", async () => {
    ...
    expect(result.results).toEqual([expect.objectContaining({ ok: true, path: "00_设定集/.agent/story-planning.jsonl" })]);
