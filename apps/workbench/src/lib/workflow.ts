...
export type PendingGeneratedSave = {
  ...
  messageId?: string;
  runId?: string;
  /** Caches generated by one batch or compound workflow share this id. */
  groupId?: string;
  createdAt?: string;
  error?: string;
};
...
export function pendingSaveFromSkill(result?: SkillRunResponse | null, source: PendingGeneratedSave["source"] = "skill"): PendingGeneratedSave | null {
  ...
  return {
    ...
    groupId: String(data.group_id || "").trim() || undefined
  };
}

/** Batch workflows return one cache per chapter. Project them into the same
 * review queue used by ordinary generated files. */
export function pendingSavesFromSkill(result?: SkillRunResponse | null, source: PendingGeneratedSave["source"] = "skill"): PendingGeneratedSave[] {
  const direct = pendingSaveFromSkill(result, source);
  ...
  return rows.flatMap((row) => {
    ...
    const pending = pendingSaveFromSkill({
      ...
      data: {
        ...data,
        pending_save: true,
        skill_id: String(data.skill_id || "body_generate")
      }
    } as SkillRunResponse, source);
