import { ClipboardCheck, Command, Save, Search, User } from "lucide-react";
...
import { PendingReviewDrawer } from "./features/product/shared/PendingReviewDrawer.js";
...
export function App() {
  ...
  return (
    <AppShell
      ...
      topbar={<WorkbenchTopbar feature={feature} projectName={controller.snapshot?.currentProject.name || ""} onSelectFeature={selectFeature} onOpenCommand={() => setCommandOpen(true)} onOpenPendingReviews={() => setPendingReviewOpen(true)} pendingReviewCount={controller.pendingReviews.length} onSave={() => void controller.saveAllDocuments()} saving={controller.documentBusy} />}
      ...
      dialog={(
        <>
          ...
          <PendingReviewDrawer
            ...
            onOpenOrigin={(review) => {
              const conversationId = review.pending.conversationId;
              if (!conversationId) return;
              navigate({ feature: "conversations" });
              void controller.loadConversation(conversationId).then(() => {
                window.setTimeout(() => document.querySelector<HTMLElement>(`[data-message-id="${review.pending.messageId || ""}"]`)?.scrollIntoView({ block: "center", behavior: "smooth" }), 60);
              });
              setPendingReviewOpen(false);
            }}
            onOpenTarget={(path) => { navigate({ feature: "editor" }); void controller.openDocument(path); setPendingReviewOpen(false); }}
            onOpenEditor={() => { navigate({ feature: "editor" }); setPendingReviewOpen(false); }}
            onOpenLibrary={(domain) => { navigate({ feature: domain === "lore" ? "sources" : "style" }); setPendingReviewOpen(false); }}
            onRequestRevision={(review) => {
              const conversationId = review.pending.conversationId;
              if (conversationId) {
                navigate({ feature: "conversations" });
                void controller.loadConversation(conversationId).then(() => controller.setMessageInput(`请基于待确认的${review.kind === "library_group" ? "资料库草稿" : "生成内容"}进行修改，保留已确认的项目上下文：`));
              }
              setPendingReviewOpen(false);
            }}
          />
...
function WorkbenchTopbar({ feature, projectName, onSelectFeature, onOpenCommand, onOpenPendingReviews, pendingReviewCount, onSave, saving }: { feature: UserFeature; projectName: string; onSelectFeature: (feature: UserFeature) => void; onOpenCommand: () => void; onOpenPendingReviews: () => void; pendingReviewCount: number; onSave: () => void; saving: boolean }) {
  return (
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        <button className="task-trigger pending-review-trigger" type="button" onClick={onOpenPendingReviews}><ClipboardCheck size={15} />待确认{pendingReviewCount > 0 && <b>{pendingReviewCount > 99 ? "99+" : pendingReviewCount}</b>}</button>
