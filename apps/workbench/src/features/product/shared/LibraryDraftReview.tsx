...
export function LibraryDraftReview({ controller, domains, onChanged, refreshKey = "", compact = false }: LibraryDraftReviewProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Compound drafts have one atomic confirmation card in the assistant and global queue.
      // Keep this legacy surface for independent drafts only.
      setDrafts((payload.drafts || []).filter((draft) => allowed.has(draft.domain) && !draft.group_id));
      setLoadError("");
    } catch (error) {
      setDrafts([]);
