...
describe("handleGeneratedCacheRoutes", () => {
  ...
  it("returns an exact target preview and rejects a stale confirmation", async () => {
    const cacheId = "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6";
    const service = new GeneratedCacheService({ projectRoot: tempDir, idFactory: () => cacheId });
    await fs.mkdir(path.join(tempDir, "02_正文"), { recursive: true });
    await fs.writeFile(path.join(tempDir, "02_正文", "第一章.txt"), "旧正文", "utf8");
    await service.create({ source: "chat", skill_id: "chat_generated", target_paths: ["02_正文/第一章.txt"], mode: "replace" });
    await service.replace(cacheId, "新正文");

    const previewJson = vi.fn();
    const previewDeps = createDeps(previewJson);
    vi.mocked(previewDeps.readJsonBody).mockResolvedValue({ mode: "replace", target_paths: ["02_正文/第一章.txt"] });
    await handleGeneratedCacheRoutes({ method: "POST" } as IncomingMessage, {} as ServerResponse, `/api/agent/generated/cache/${cacheId}/preview`, createContext(), previewDeps);
    const preview = previewJson.mock.calls[0]?.[2] as { targets: Array<{ before_content: string; after_content: string; before_hash: string }> };
    expect(preview.targets[0]).toMatchObject({ before_content: "旧正文", after_content: "新正文" });

    await fs.writeFile(path.join(tempDir, "02_正文", "第一章.txt"), "外部改写", "utf8");
    const saveJson = vi.fn();
    const saveDeps = createDeps(saveJson);
    vi.mocked(saveDeps.readJsonBody).mockResolvedValue({
      cache_id: cacheId,
      skill_id: "chat_generated",
      mode: "replace",
      target_paths: ["02_正文/第一章.txt"],
      target_path: "",
      chapter: 0,
      expected_target_hashes: { "02_正文/第一章.txt": preview.targets[0]!.before_hash }
    });
    await handleGeneratedCacheRoutes({ method: "POST" } as IncomingMessage, {} as ServerResponse, "/api/agent/generated/save", createContext(), saveDeps);
    expect(runtimeMocks.commitGeneratedCache).not.toHaveBeenCalled();
    expect(saveJson).toHaveBeenCalledWith(expect.anything(), 409, expect.objectContaining({ code: "GENERATED_PREVIEW_STALE" }));
  });
});
