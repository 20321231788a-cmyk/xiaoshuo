...
export async function handleProjectLibraryRoutes(
  ...
  if (!pathname.startsWith("/api/project-libraries") && !pathname.startsWith("/api/project-library-drafts") && !pathname.startsWith("/api/project-library-draft-groups")) {
    return false;
  }
  ...
  if (request.method === "GET" && pathname === "/api/project-library-draft-groups") {
    try {
      deps.writeJson(response, 200, { groups: await service.listDraftGroups() });
    } catch (error) {
  ...
  const draftGroupPreview = /^\/api\/project-library-draft-groups\/([^/]+)\/preview$/.exec(pathname);
  if (draftGroupPreview && request.method === "GET") {
    try {
      deps.writeJson(response, 200, await service.previewDraftGroup(decodeURIComponent(draftGroupPreview[1] || "")));
    } catch (error) {
      writeLibraryError(response, error, deps.writeJson);
    }
    return true;
  ...
  const draftGroupOrigin = /^\/api\/project-library-draft-groups\/([^/]+)\/origin$/.exec(pathname);
  if (draftGroupOrigin && request.method === "PUT") {
    try {
      const payload = await deps.readJsonBody(request);
      deps.writeJson(response, 200, await service.setDraftGroupOrigin(decodeURIComponent(draftGroupOrigin[1] || ""), {
  ...
  const draftGroupCommit = /^\/api\/project-library-draft-groups\/([^/]+)\/commit$/.exec(pathname);
  if (draftGroupCommit && request.method === "POST") {
    try {
      const bundles = await service.commitDraftGroup(decodeURIComponent(draftGroupCommit[1] || ""), deps.ensureDocumentSession(context.documentSessions, current.path));
  ...
  const draftGroupDelete = /^\/api\/project-library-draft-groups\/([^/]+)$/.exec(pathname);
  if (draftGroupDelete && request.method === "DELETE") {
    try {
      await service.discardDraftGroup(decodeURIComponent(draftGroupDelete[1] || ""));
